...
if [ -f $partition1$file ]; then

 mv $partition1$file $partition2 && mv  $partition2$file $partition3
 echo "$(date +%Y%m%d%H%M%S) movendo o arquivo $partition1$file para a partição $partition2 e movendo da $partition2 para a $partition3" >> $log
